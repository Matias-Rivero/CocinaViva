...
<body>
	...
	<div id="page">
			...
			<div class="gtco-container">
				...
				<div class="row">
					<div class="col-sm-1 col-xs-12">
						<div id="gtco-logo">
							<a href="home"><img src="images/logo.png" width="170px"
								height="60px" style="margin: -10px" /></a>
						</div>
					</div>
						...
						<c:when test="${usuariologueado != null}">
							<div class="col-xs-11 text-right menu-1">
								<ul>
									...
									<li><a href="ingredientes"><span>Agregar
		...
		<div class="gtco-section">
			<div class="gtco-container">
				<div class="py-5 text-center" id="gtco-counter">
					...
					<form:form id="form1" method="POST" modelAttribute="lingrediente"
						action="drecetas">
						<label class="oculto"><form:input path="id" id="id"
								type="text" class="form-control" /></label>
						<label class="oculto"><form:input path="nombre"
								id="nombrereceta" type="text" class="form-control" /></label>
						<label class="oculto"><form:input path="gastouser"
								id="ireALaParte" type="text" class="form-control" /></label>
						<c:forEach items="${ingredinetesseleccionados}"
							var="ingredinetesseleccionados">
							<label class="label label-success"><a
								href="javascript:quitarIng(${ingredinetesseleccionados.id});"
								class="badge badge-danger">x</a>${ingredinetesseleccionados.nombre}<span
							...
							<label class="oculto"><form:input path="seleccionados"
									type="text" value="${ingredinetesseleccionados.id}" /></label>
						</c:forEach>
					</form:form>
				</div>
				...
				<div class="row">
					<div class="col-md-12 order-md-12 mb-12">
						...
						<ul class="list-group mb-3">
							...
							<li
								...
								<form name="buscar" action="javascript:;"
									onsubmit="buscarRecetas(this)" method="get">
									<div class="input-group">

										<span class="input-group-addon"><a
											href="javascript:buscarRecetas();">Buscar</a></span> <input
											type="text" id="buscarR" class="form-control" minlength="4"
											maxlength="30"
											value="<c:if test="${not empty loquebusco}">${loquebusco}</c:if>"
											placeholder="estofado de pollo" autocomplete="off">

									</div>
								</form>
							</li>
							<c:if test="${not empty nohayrecetascriterio}">
								<li><label class="label label-info">No hay recetas
										con el criterio ${nohayrecetascriterio}</label></li>
							</c:if>
							<c:if test="${not empty loquebusco}">
								<li><a class="glyphicon glyphicon-remove"
									...
									<div class="col-lg-4 col-md-4 col-sm-6">
										...
										<a href="leerRecetas?id=${listaRecetas.id}"
											...
											<div class="fh5co-text">
												...
												<p>
													<span class="label">Calorias:</span> <label>${listaRecetas.calorias}</label>
												</p>
												<div class="table-responsive">
													<table class="table table-bordered">
														<tbody>
															<!-- Aplicadas en las filas -->
															<tr class="active">Requiere:
															</tr>

															<!-- Aplicadas en las celdas (<td> o <th>) -->
															<c:forEach items="${listaRecetas.listaIngrediente}"
																var="listaIngredientes">
																<tr>
																	<td class="active">${listaIngredientes.nombre}
																		${listaIngredientes.cantidadstring}</td>
																	<c:if test="${listaIngredientes.faltante > 0}">
																		<td class="success"><label
																			class="label label-success">OK</label></td>
																	</c:if>
																	<c:if test="${listaIngredientes.faltante < 0}">
																		<td class="info"><label class="label label-info">FALTAN
																				${listaIngredientes.faltante * -1} </label></td>
																	</c:if>
																	<c:if test="${listaIngredientes.faltante == 0}">
																		<td class="warning"><label
																			class="label label-warning">OK</label></td>
																	</c:if>
																	<c:if test="${empty listaIngredientes.faltante}">
																		<td class="danger"><label
																			class="label label-danger">NO</label></td>
																	</c:if>
																</tr>
															</c:forEach>
														</tbody>
					...
					<c:if test="${empty listaRecetas && empty listaRecetasBuscadas}">
						<div class="text-center">
							<label class="label label-danger">Lo sentimos - todavía
			...
			<div id="buscarrecetasvacio" class="modal fade" tabindex="-1"
		...
		<script type="text/javascript">
			function iraParte(parte) {
				document.getElementById("ireALaParte").value = parte;
